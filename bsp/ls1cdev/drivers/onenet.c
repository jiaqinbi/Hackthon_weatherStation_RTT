...
备    注：	无
**************************************************************/
void post_data_process(char *dev_id, char *api_key, char *datastream_id, int point_value)
{
	...
	sprintf(post_content,"{\"datastreams\":[{\"id\":\"%s\",\"datapoints\":[{\"value\":%d.%d}]}]}", (char*)datastream_id,  (int)point_value/10, (int) point_value % 10);
...
{
	post_data_process(dev_id, api_key, datastream_id, point_value);
...
void get_data_stream_from_onenet(char *dev_id, char *api_key, char *datastream_id)
{
	get_data_process(dev_id, api_key, datastream_id);
